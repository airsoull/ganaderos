...
foreach ($model as $m) {
	...
	<?PHP 
		if($cat != $m->id_vina){ 
			#echo '<tr><td colspan="4"><a name="'.str_replace(' ','',$m->idCategoriaalimentos->nombre).'"></a><h2>'.ucwords($m->idCategoriaalimentos->nombre).'</h2><br></td></tr>'; 
	...
	<tr>
    	...
    	<td width="20%" valign="top"><input name="<?PHP echo $m->id_vinos; ?>" type="button" value="-" class="menos"><input name="<?PHP echo $m->id_vinos; ?>" type="text" size="2" maxlength="2" value="0" class="numero" dinero="<?PHP echo $m->precio; ?>" readonly><input name="<?PHP echo $m->id_vinos; ?>" type="button" value="+" class = "mas"></td>
	</tr>
...
<div class="five columns offset-by-one">
	...
	<h3>Total:</h3>
		...
		<?php /*
			foreach($tipo as $t){
				echo CHtml::link($t->idCategoriaalimentos->nombre, '#'.str_replace(' ','',$t->idCategoriaalimentos->nombre), array('optionName'=>'optionValue')).'<br>';
			}	*/ 
		?>
	</div>
</div>
